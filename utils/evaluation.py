...
def eval_mask_slice(valloader, model, criterion, opt, args):
    ...
    for batch_idx, (datapack) in enumerate(valloader):
        ...
        for j in range(0, b):
            ...
            gt_i = np.zeros((1, h, w))
            gt_i[gt[j:j+1, :, :] == 1] = 255
            dices[1] += metrics.dice_coefficient(pred_i, gt_i)
            ious[1] += metrics.iou_coefficient(pred_i, gt_i)
            hds[1] += metrics.hausdorff_95(pred_i[0, :, :], gt_i[0, :, :])
            del pred_i, gt_i
    ...
    if opt.mode == "train":
        ...
        return mean_dice, mean_iou, mean_hdis
...
def eval_mask_slice2(valloader, model, criterion, opt, args):
    ...
    for batch_idx, (datapack) in enumerate(valloader):
        ...
        for j in range(0, b):
            ...
            ious[eval_number+j, 1] += metrics.iou_coefficient(pred_i, gt_i)
            hds[eval_number+j, 1] += metrics.hausdorff_95(pred_i[0, :, :], gt_i[0, :, :])
            del pred_i, gt_i
            if opt.visual:
    ...
    if opt.mode == "train":
        ...
        return dice_mean, hd_mean, iou_mean, dices_std, hd_std, iou_std
...
def eval_camus_patient(valloader, model, criterion, opt, args):
    ...
    fns = np.zeros((patientnumber, opt.classes))
    ...
    for batch_idx, (datapack) in enumerate(valloader):
        ...
        for j in range(0, b):
            ...
            hd = metrics.hausdorff_95(pred_i[0, :, :], gt_i[0, :, :])
...
def eval_camus_samed(valloader, model, criterion, opt, args):
    ...
    fns = np.zeros((patientnumber, classes))
